...
fn is_byte(value: i16) -> bool {
    0 <= value && value < 256
}

fn make_color(r: i16, g: i16, b: i16) -> Option<Color> {
    if is_byte(r) && is_byte(g) && is_byte(b) {
        let c = Color { red: r as u8, blue: b as u8, green: g as u8 };
        Some(c)
    }
    else {
        None
...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        let (r, g, b) = tuple;
        if let Some(c) = make_color(r, g, b) {
            Ok(c)
        }
        else {
